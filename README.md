...
# Arp view (default):
all controls apply to currently selected layer only
  ...
  tap bottom row: mute/skip toggle
  long press bottom row: delete
...
# lap view (hold K1)
  ...
  long press bottom row: insert/replace selected note. 
  tap again to toggle between insert/replace.
  press anywhere else to abort

# phrase view (press/hold K2):
...
# parameters:
  ...
  collision: determines what happens when two layers play the same note on the same device and channel. 
	  none: no handling, causes some synths drop notes
